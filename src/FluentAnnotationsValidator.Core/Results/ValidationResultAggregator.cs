...
public static class ValidationResultAggregator
{
    ...
    IDictionary<object, object?>? contextItems = null)
    {
        ...
        foreach (var rule in ruleList)
        {
            ...
            {
                case IFluentRuleAttribute fluentAttr:
    ...
    IValidationMessageResolver resolver, IRuleRegistry registry)
    {
        ...
        if (attr is IFluentValidationAttribute fva)
        {
            fva.Rule = rule;
    ...
    private static void ProcessFluentRule(IFluentRuleAttribute attribute,
    ...
    {
        if (attribute is not ValidationAttribute attr)
            throw new InvalidCastException("Object must be a validation attribute.");
        
        var isUnresolvedMessage = CheckFluentValidationAttribute(attr, rule, resolver, registry);
        var context = new ValidationContext(instance, contextItems) { MemberName = member.Name };
        var fluentResult = attr.GetValidationResult(instance, context);
        if (fluentResult != ValidationResult.Success)
        {
            AddErrors(errors, instance, member, attr, memberValue, rule, fluentResult, resolver, isUnresolvedMessage);
        }
    }

    private static async Task ProcessFluentRuleAsync(IFluentRuleAttribute attribute,
    object instance, MemberInfo member, object? memberValue,
    IDictionary<object, object?>? contextItems,
