...
public class FluentTypeValidator<T>(FluentTypeValidatorRoot root)
    : FluentTypeValidatorBase(typeof(T)), IFluentTypeValidator<T>, IDisposable
{
    ...
    /// <inheritdoc cref="IFluentTypeValidator{T}.AlwaysValidate()"/>
    public virtual FluentTypeValidator<T> AlwaysValidate()
        => When(AlwaysTruePredicate);
    ...
    IFluentTypeValidator<T> IFluentTypeValidator<T>.AlwaysValidate()
        => AlwaysValidate();
    ...
    /// <summary>
    /// Releases resources used by the object. If <paramref name="disposing"/> is <c>true</c>, 
    /// invokes <see cref="Build"/> to finalize configuration before disposal.
    /// </summary>
