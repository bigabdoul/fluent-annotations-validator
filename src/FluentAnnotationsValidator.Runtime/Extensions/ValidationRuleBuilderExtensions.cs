...
public static class ValidationRuleBuilderExtensions
{
    ...
    Action<FluentTypeValidator<TProp>> configure)
    {
        ...
        CollectionValidationAttribute<TProp> attr = builder.IsAsync
            ? new CollectionAsyncAttribute<TProp>
            {
                RuleRegistry = builder.Registry,
            }
            : new CollectionAttribute<TProp>
            {
                RuleRegistry = builder.Registry,
            };
        ...
        Expression<Func<T, TProp>> otherProperty, ComparisonOperator comparison = ComparisonOperator.Equal)
    {
        ...
        return builder.SetValidator(new ComparisonAttribute(otherPropertyName, comparison));
    }
    ...
    public static IValidationRuleBuilder<T, TProp> Compare<T, TProp>(this IValidationRuleBuilder<T, TProp> builder,
        ...
        => builder.SetValidator(new ComparisonAttribute(otherProperty, comparison));
    ...
    public static IValidationRuleBuilder<T, TProp> Length<T, TProp>(this IValidationRuleBuilder<T, TProp> builder,
        int min, int max)
        => builder.SetValidator(new LengthCountAttribute(min, max));
    ...
    internal static ValidationRule<T> CreateValidationRule<T>(this IValidationRule rule, MemberInfo member)
    {
        return new ValidationRule<T>(
            condition: instance => rule.Condition(instance!),
            rule.Expression,
            rule.Message,
            rule.Key,
            rule.ResourceKey,
