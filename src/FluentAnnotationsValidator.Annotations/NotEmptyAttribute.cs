...
public sealed class NotEmptyAttribute : FluentValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        if (value is null)
            return GetFailedValidationResult(this, validationContext, MessageResolver);

        // For strings, consider null, empty, or whitespace as invalid.
        if (value is string s)
        {
            return string.IsNullOrWhiteSpace(s) ? GetFailedValidationResult(this, validationContext, MessageResolver) : ValidationResult.Success;
        }
