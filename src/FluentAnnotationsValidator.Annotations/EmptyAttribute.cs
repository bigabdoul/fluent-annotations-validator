...
public sealed class EmptyAttribute : FluentValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        {
            return string.IsNullOrWhiteSpace(s) ? ValidationResult.Success : GetFailedValidationResult(this, validationContext, MessageResolver);
        }

        // For other types, check if a count can be obtained and if it is zero.
        if (CountHelper.TryGetCount(value, out int length))
        {
            return length == 0 ? ValidationResult.Success : GetFailedValidationResult(this, validationContext, MessageResolver);
        }
