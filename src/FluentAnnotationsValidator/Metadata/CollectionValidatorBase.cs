...
public abstract partial class CollectionValidatorBase<T> : FluentValidationAttribute, ICollectionValidationResult
{
    ...
    protected IEnumerable<FluentValidationFailure> ProcessErrors(IEnumerable<FluentValidationFailure> errors, string segment)
    {
        ...
        {
            err.CollectionIndex = err.CollectionIndex < 0 ? collectionIndex : err.CollectionIndex;
            err.ParentCollectionIndex = err.ParentCollectionIndex < 0 ? parentIndex : err.ParentCollectionIndex;
    ...
    private static (int collectionIndex, int parentIndex) ExtractIndices(string? segment)
    {
        ...
        int collectionIndex = -1, parentIndex = -1;
